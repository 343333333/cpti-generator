import os,json,time
from qgen.dispatcher.LSF import LSF_sub,LSF_head_gene,LSF_body_gene,LSF_tail_gene
from qgen.dispatcher.PBS import PBS_sub,PBS_head_gene,PBS_body_gene,PBS_tail_gene
...
def do_sub(sub_para,dir_index):
	if sub_para['queue_system']=='LSF' :
		...
		os.system('bsub -J %s < ./mission.sub &'%('qgen'))
		os.chdir(os.path.dirname(os.getcwd()))
	elif sub_para['queue_system']=='PBS' :
		os.chdir('./%s'%dir_index)
		os.system('qsub -N %s  ./mission.sub & '%('qgen' ))
		os.chdir(os.path.dirname(os.getcwd()))
