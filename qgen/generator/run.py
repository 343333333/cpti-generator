...
from qgen.generator.pathfinder import make_iter_path,make_step_path,gene_path_name,make_multi_path,make_path
from qgen.generator.generator import step_rec,step_log,keyword_replace,keyword_exract
from qgen.generator.iter_bader import INCAR_read,new_POSCAR_gene,atom_num_get
from qgen.generator.V_cal import V_cal,get_xyz,V2N
from qgen.dispatcher.Dispatcher import job_sub,head_gene,body_gene,tail_gene
	...
	for i in range(n_max):
		if i%bader_each_file == 0 :
			...
			with open(sub_file_path+'/mission.sub','a+') as fp :
				...
				fp.write(tail_gene(sub_para)) if (i+1)%bader_each_file==0 else 0
		else :
			sub_file_path=os.path.join(step_path, dir_format %dir_index)
			bader_path=os.path.join(sub_file_path,dir_format %i)
			make_path(bader_path)
...
def do_ti(iter_num,para):
	...
	try:
		...
		print('the MD_traj doesnt exists in the path %s\n plz check your file \n'%(POSCAR_path))
		raise Exception(' ')
